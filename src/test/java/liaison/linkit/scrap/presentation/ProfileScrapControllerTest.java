...
public class ProfileScrapControllerTest extends ControllerTest {
    ...
    @MockBean private ProfileScrapService profileScrapService;

    private ResultActions performUpdateProfileScrap(
            final String emailId, final UpdateProfileScrapRequest request) throws Exception {
    ...
    void updateProfileScrap() throws Exception {
        ...
        final UpdateProfileScrapRequest updateProfileScrapRequest =
                UpdateProfileScrapRequest.builder().changeScrapValue(true).build();

        final ProfileScrapResponseDTO.UpdateProfileScrap updateProfileScrap =
                UpdateProfileScrap.builder().emailId("이메일 아이디").isProfileScrap(true).build();

        // when
        when(profileScrapService.updateProfileScrap(anyLong(), any(), any()))
                .thenReturn(updateProfileScrap);

        final ResultActions resultActions =
                performUpdateProfileScrap("liaison", updateProfileScrapRequest);
                                ...
                                restDocs.document(
                                        ...
                                        responseFields(
                                                ...
                                                fieldWithPath("result.isProfileScrap")
                                                        .type(JsonFieldType.BOOLEAN)
                                                        .description(
                                                                "변경된 프로필 스크랩 여부 (true -> 스크랩한 상태 / false -> 스크랩하지 않은 상태)"))))
                        .andReturn();

        final String jsonResponse = mvcResult.getResponse().getContentAsString();
        final CommonResponse<UpdateProfileScrap> actual =
                objectMapper.readValue(
                        jsonResponse, new TypeReference<CommonResponse<UpdateProfileScrap>>() {});

        final CommonResponse<UpdateProfileScrap> expected =
                CommonResponse.onSuccess(updateProfileScrap);
