...
public class ProfileLinkControllerTest extends ControllerTest {
    ...
    private ResultActions performUpdateProfileLink(final AddProfileLinkRequest profileLinkRequest) throws Exception {
        return mockMvc.perform(
    ...
    void getProfileLinkItems() throws Exception {
        ...
        final CommonResponse<ProfileLinkItems> actual = objectMapper.readValue(
                jsonResponse,
                new TypeReference<CommonResponse<ProfileLinkItems>>() {
                }
        );

        final CommonResponse<ProfileLinkItems> expected = CommonResponse.onSuccess(profileLinkItems);
    ...
    void updateProfileLinkItems() throws Exception {
        ...
        final ProfileLinkRequestDTO.AddProfileLinkRequest addProfileLinkRequest
                ...
                .profileLinkItems(Arrays.asList(firstProfileLinkItem, secondProfileLinkItem))
                .build();
        ...
        final ProfileLinkResponseDTO.ProfileLinkItems profileLinkItems
                ...
                .profileLinkItems(Arrays.asList(firstProfileLinkItemRequest, secondProfileLinkItemRequest))
                .build();

        // when
        when(profileLinkService.updateProfileLinkItems(anyLong(), any())).thenReturn(profileLinkItems);
        ...
        // JSON 응답에서 result 객체를 추출 및 검증
        final String jsonResponse = mvcResult.getResponse().getContentAsString();
        final CommonResponse<ProfileLinkResponseDTO.ProfileLinkItems> actual = objectMapper.readValue(
                jsonResponse,
                new TypeReference<CommonResponse<ProfileLinkItems>>() {
                }
        );
