...
class ProfileControllerTest extends ControllerTest {
    ...
    private ResultActions performGetProfileDetail(final Long profileId) throws Exception {
        return mockMvc.perform(
                RestDocumentationRequestBuilders.get("/api/v1/profile/{profileId}", profileId)
        );
    }
    ...
    @Test
    void getProfileMyDetail() throws Exception {
        // given
        final ProfileCompletionMenu profileCompletionMenu = new ProfileCompletionMenu(100);
    ...
    @DisplayName("회원이 남이 보는 나의 프로필을 조회할 수 있다.")
    @Test
    void getProfileDetail() throws Exception {
        ...
        // when
        when(profileService.getProfileDetail(anyLong())).thenReturn(profileDetail);
