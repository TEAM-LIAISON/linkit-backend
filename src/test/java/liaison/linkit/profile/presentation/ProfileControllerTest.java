...
class ProfileControllerTest extends ControllerTest {
    ...
    private ResultActions performGetProfileDetail(final Long profileId) throws Exception {
        return mockMvc.perform(
    ...
    @DisplayName("회원이 내가 보는 나의 프로필을 조회할 수 있다.")
    @Test
    void getProfileDetail() throws Exception {
        ...
        final List<ProfileLogResponseDTO.ProfileLogItem> profileLogItems
                = Arrays.asList(firstProfileLogItem, secondProfileLogItem);
        ...
        // when
        when(profileService.getProfileDetail(anyLong(), anyLong())).thenReturn(profileDetail);

        final ResultActions resultActions = performGetProfileDetail(1L);
                ...
                .andDo(
                        restDocs.document(
                                ...
                                responseFields(
                                        ...
                                        fieldWithPath("result.profileLogItems")
                                                .type(JsonFieldType.ARRAY)
                                                .description("프로필 로그 항목 배열"),
