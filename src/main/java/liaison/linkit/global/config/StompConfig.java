...
public class StompConfig implements WebSocketMessageBrokerConfigurer {
    ...
    /**
     * 메시지 브로커 설정 - 서버와 클라이언트 간 메시지 송수신 경로를 정의 - "/app"으로 시작하는 경로는 애플리케이션으로 전달 - "/topic"은 브로드캐스트용
     * 경로 (예: 채팅방 메시지)
     */
    @Override
    public void configureMessageBroker(MessageBrokerRegistry config) {
        config.enableSimpleBroker("/sub/chat", "/sub/notification/header", "/sub/chat/read");
    ...
    /**
     * STOMP 엔드포인트 설정 - 클라이언트가 WebSocket 서버에 연결하기 위한 엔드포인트 - SockJS를 활성화하여 WebSocket을 지원하지 않는
     * 브라우저에서도 사용 가능 - CORS 허용을 위해 `setAllowedOrigins("*")` 및 `setAllowedOriginPatterns("*")` 사용
     */
    @Override
    ...
    /**
     * 클라이언트에서 서버로 들어오는 메시지 채널 설정 - StompHandler를 인터셉터로 등록하여 메시지 인증 및 추가 처리를 수행 - 예: 인증 토큰 확인, 사용자
     * 권한 확인
     */
    @Override
    public void configureClientInboundChannel(ChannelRegistration registration) {
        registration.interceptors(stompHandler); // StompHandler를 인터셉터로 추가
        registration.taskExecutor().corePoolSize(4).maxPoolSize(10).queueCapacity(50);
    }
