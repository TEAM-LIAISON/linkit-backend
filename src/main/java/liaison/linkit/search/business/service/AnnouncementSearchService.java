...
public class AnnouncementSearchService {
    ...
    public AnnouncementSearchResponseDTO searchAnnouncements(
        ...
        if (isDefaultSearch) {
            ...
            List<TeamMemberAnnouncement> hotAnnouncements = teamMemberAnnouncementQueryAdapter.findHotAnnouncements(hotPageable).getContent();

            List<AnnouncementInformMenu> hotAnnouncementDTOs = hotAnnouncements.stream()
                .map(teamMemberAnnouncement -> announcementInformMenuAssembler.mapToAnnouncementInformMenu(teamMemberAnnouncement, optionalMemberId))
                .toList();
            ...
            Page<TeamMemberAnnouncement> remainingAnnouncements = teamMemberAnnouncementQueryAdapter.findExcludedAnnouncements(excludeAnnouncementIds, pageable);
            Page<AnnouncementInformMenu> remainingAnnouncementDTOs = remainingAnnouncements.map(teamMemberAnnouncement ->
                announcementInformMenuAssembler.mapToAnnouncementInformMenu(teamMemberAnnouncement, optionalMemberId)
            );
            ...
            Page<AnnouncementInformMenu> announcementDTOs = announcements.map(teamMemberAnnouncement ->
                announcementInformMenuAssembler.mapToAnnouncementInformMenu(teamMemberAnnouncement, optionalMemberId)
            );
