...
public class ProfileSearchService {
     ...
     * 팀원 찾기 화면의 검색 로직
     * <p>
     * - 쿼리 파라미터가 전혀 없으면 기본 검색으로 간주하여: 1. 상단: 프로필 완성도가 높은 팀원 6명(예: completionScore 기준 내림차순) 2. 하단: 상단에 포함된 프로필을 제외한 나머지 팀원들을 최신순(createdAt 내림차순)으로 페이지네이션 - 필터 쿼리 파라미터가 있으면 기존의 필터링 로직대로 검색합니다.
     *
    ...
    public ProfileSearchResponseDTO searchProfiles(
        ...
        if (isDefaultSearch) {
            ...
            List<Profile> topProfiles = profileQueryAdapter.findTopCompletionProfiles(topPageable).getContent();

            List<ProfileInformMenu> topProfileDTOs = topProfiles.stream()
                .map(profile -> profileInformMenuAssembler.assembleProfileInformMenu(profile, optionalMemberId))
                .collect(Collectors.toList());

            // 2. 하단 영역: 상단에 포함된 프로필 제외 후 최신순(createdAt 내림차순) 페이지네이션
            List<Long> excludeIds = topProfiles.stream()
                .map(Profile::getId)
                .collect(Collectors.toList());
