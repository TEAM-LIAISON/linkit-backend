...
public class TeamDetailAssembler {
     ...
     */
    public TeamDetail assembleTeamDetail(
        ...
        LoginTeamFlags loginFlags =
                optionalMemberId
                        .map(memberId -> assembleLoginTeamFlags(memberId, targetTeam))
                        .orElse(new LoginTeamFlags());
    ...
    private LoginTeamFlags assembleLoginTeamFlags(final Long memberId, final Team targetTeam) {
        ...
        flags.isMyTeam =
                teamMemberQueryAdapter.isOwnerOrManagerOfTeam(targetTeam.getId(), memberId);
        flags.isTeamInvitationInProgress =
                teamMemberInvitationQueryAdapter.existsByEmailAndTeam(
                        member.getEmail(), targetTeam);
        flags.isTeamDeleteRequester =
                teamMemberQueryAdapter.isTeamDeleteRequester(member.getId(), targetTeam.getId());
        flags.isTeamDeleteInProgress =
                teamQueryAdapter.isTeamDeleteInProgress(targetTeam.getTeamCode()) && flags.isMyTeam;
