...
public class AnnouncementInformMenuAssembler {
    ...
    /**
     * 홈 화면에 표시할 공고 목록(AnnouncementInformMenus)을 조립하여 반환합니다. 로그인 상태(Optional
     * memberId 값 존재)와 로그아웃 상태(Optional.empty()) 모두 처리합니다.
     *
     * @param optionalMemberId 로그인한 회원의 ID(Optional). 값이 있으면 로그인 상태, 없으면 로그아웃 상태로
     *                         처리합니다.
    ...
    public AnnouncementInformMenus assembleHomeAnnouncementInformMenus(final Optional<Long> optionalMemberId) {
        ...
        List<TeamMemberAnnouncement> announcements = teamMemberAnnouncementQueryAdapter
                .findTopTeamMemberAnnouncements(9);
    ...
    /**
     * 공고(TeamMemberAnnouncement)를 AnnouncementInformMenu DTO로 변환합니다. 로그인
     * 상태(Optional 값 존재)와 로그아웃 상태를 모두 처리할 수 있습니다.
     *
