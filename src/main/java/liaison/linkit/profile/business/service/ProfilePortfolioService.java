...
public class ProfilePortfolioService {
    ...
    @Transactional(readOnly = true)
    public ProfilePortfolioResponseDTO.ProfilePortfolioItems getProfilePortfolioViewItems(
    ...
    public ProfilePortfolioResponseDTO.ProfilePortfolioDetail getProfilePortfolioDetailInLoginState(
        ...
        final List<ProjectRoleContribution> projectRoleContributions =
                projectRoleContributionQueryAdapter.getProjectRoleContributions(profilePortfolioId);
        final List<ProjectRoleAndContribution> projectRoleAndContributions =
                profilePortfolioMapper.toProjectRoleAndContributions(projectRoleContributions);

        // 해당 포트폴리오(프로젝트)의 연결된 사용 스킬 조회
        final List<ProjectSkill> projectSkills =
                projectSkillQueryAdapter.getProjectSkills(profilePortfolio.getId());
        final List<ProjectSkillName> projectSkillNames =
                profilePortfolioMapper.toProjectSkillNames(projectSkills);

        // 해당 포트폴리오(프로젝트)의 연결된 이미지 조회
        final List<String> projectSubImagePaths =
                projectSubImageQueryAdapter.getProjectSubImagePaths(profilePortfolio.getId());
        final PortfolioImages portfolioImages =
                profilePortfolioMapper.toPortfolioImages(
                        profilePortfolio.getProjectRepresentImagePath(), projectSubImagePaths);

        final List<ProjectLink> projectLinks =
                projectLinkQueryAdapter.getProjectLinks(profilePortfolio.getId());
        final List<ProjectLinkNameAndUrls> projectLinkNameAndUrls =
                profilePortfolioMapper.toProjectLinks(projectLinks);
    ...
    @Transactional(readOnly = true)
    public ProfilePortfolioResponseDTO.ProfilePortfolioDetail
    ...
    public ProfilePortfolioResponseDTO.AddProfilePortfolioResponse addProfilePortfolio(
        ...
        final ProfilePortfolio profilePortfolio =
                profilePortfolioMapper.toAddProfilePortfolio(profile, addProfilePortfolioRequest);
        final ProfilePortfolio savedProfilePortfolio =
                profilePortfolioCommandAdapter.addProfilePortfolio(
                        profilePortfolio); // 포트폴리오 객체 우선 저장

        // 대표 이미지 저장
        if (imageValidator.validatingImageUpload(projectRepresentImage)) {
            projectRepresentImagePath =
                    s3Uploader.uploadProfileProjectRepresentImage(
        ...
        if (projectSubImages != null && !projectSubImages.isEmpty()) {
            ...
            for (MultipartFile subImage : projectSubImages) {
                if (imageValidator.validatingImageUpload(subImage)) {
                    String subImagePath =
                            s3Uploader.uploadProfileProjectSubImage(new ImageFile(subImage));
                    projectSubImagePaths.add(subImagePath);
                    ProjectSubImage projectSubImage =
                            ProjectSubImage.builder()
        ...
        final PortfolioImages portfolioImages =
                profilePortfolioMapper.toPortfolioImages(
                        projectRepresentImagePath, projectSubImagePaths);

        // 역할 및 기여도 저장
        final List<ProjectRoleContribution> projectRoleContributions =
                profilePortfolioMapper.toAddProjectRoleContributions(
                        savedProfilePortfolio,
                        addProfilePortfolioRequest.getProjectRoleAndContributions());
        final List<ProjectRoleContribution> savedProjectRoleContributions =
                projectRoleContributionCommandAdapter.addProjectRoleContributions(
                        projectRoleContributions);
        final List<ProjectRoleAndContribution> projectRoleAndContributions =
                profilePortfolioMapper.toProjectRoleAndContributions(savedProjectRoleContributions);

        // 사용 스킬 저장
        List<String> skillNames =
                addProfilePortfolioRequest.getProjectSkillNames().stream()
                        .map(
                                ProfilePortfolioRequestDTO.ProjectSkillName
                                        ::getProjectSkillName) // 괄호 제거
                        .collect(Collectors.toList());

        final List<Skill> skills = skillQueryAdapter.getSkillsBySkillNames(skillNames);
        final List<ProjectSkill> projectSkills =
                profilePortfolioMapper.toAddProjectSkills(savedProfilePortfolio, skills);
        ...
        final List<ProjectLink> savedProjectLinks =
                projectLinkCommandAdapter.addProjectLinks(
                        profilePortfolioMapper.toAddProjectLinks(
                                savedProfilePortfolio,
                                addProfilePortfolioRequest.getProjectLinkNameAndUrls()));
        final List<ProjectLinkNameAndUrls> projectLinkNameAndUrls =
                profilePortfolioMapper.toProjectLinks(savedProjectLinks);
        ...
        return profilePortfolioMapper.toAddProfilePortfolioResponse(
                savedProfilePortfolio,
                projectRoleAndContributions,
                projectSkillNames,
                projectLinkNameAndUrls,
    ...
    public ProfilePortfolioResponseDTO.UpdateProfilePortfolioResponse updateProfilePortfolio(
        ...
        final ProfilePortfolio updatedProfilePortfolio =
                profilePortfolioCommandAdapter.updateProfilePortfolio(
                        existingProfilePortfolio, updateProfilePortfolioRequest);
        ...
        // (4-b) 기존 DB 서브 이미지 목록
        List<ProjectSubImage> existingSubImages =
                projectSubImageQueryAdapter.getProjectSubImages(profilePortfolioId);
        ...
        final PortfolioImages portfolioImages =
                profilePortfolioMapper.toPortfolioImages(
                        ...
                        Stream.concat(keepPaths.stream(), newProjectSubImagePaths.stream())
                                .toList());
        ...
        // =====================================
        // 기존 역할 및 기여도 삭제
        List<ProjectRoleContribution> existingRoleContributions =
                projectRoleContributionQueryAdapter.getProjectRoleContributions(profilePortfolioId);
        ...
        // =====================================
        List<ProjectSkill> existingProjectSkills =
                projectSkillQueryAdapter.getProjectSkills(profilePortfolioId);
        ...
        List<String> newSkillNames =
                updateProfilePortfolioRequest.getProjectSkillNames().stream()
        ...
        final List<ProjectSkill> newProjectSkills =
                profilePortfolioMapper.toAddProjectSkills(updatedProfilePortfolio, newSkills);
        projectSkillCommandAdapter.saveAll(newProjectSkills);
        final List<ProjectSkillName> projectSkillNames =
                profilePortfolioMapper.toProjectSkillNames(newProjectSkills);

        List<ProjectLink> existingProjectLinks =
                projectLinkQueryAdapter.getProjectLinks(profilePortfolioId);
        if (existingProjectLinks != null && !existingProjectLinks.isEmpty()) {
            projectLinkCommandAdapter.deleteAll(existingProjectLinks);
        }

        final List<ProjectLink> newProjectLinks =
                profilePortfolioMapper.toAddProjectLinks(
                        updatedProfilePortfolio,
                        updateProfilePortfolioRequest.getProjectLinkNameAndUrls());
        projectLinkCommandAdapter.addProjectLinks(newProjectLinks);
        final List<ProjectLinkNameAndUrls> projectLinkNameAndUrls =
                profilePortfolioMapper.toProjectLinks(newProjectLinks);
    ...
    public ProfilePortfolioResponseDTO.RemoveProfilePortfolioResponse removeProfilePortfolio(
            final Long memberId, final Long profilePortfolioId) {
        final Profile profile = profileQueryAdapter.findByMemberId(memberId);
        final ProfilePortfolio profilePortfolio =
                profilePortfolioQueryAdapter.getProfilePortfolio(profilePortfolioId);
