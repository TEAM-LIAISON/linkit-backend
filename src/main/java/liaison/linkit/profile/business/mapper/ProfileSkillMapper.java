...
public class ProfileSkillMapper {
    ...
    public ProfileSkillResponseDTO.ProfileSkillItem toProfileSkillItem(
            final ProfileSkill profileSkill) {
        return ProfileSkillResponseDTO.ProfileSkillItem.builder()
    ...
    public ProfileSkillResponseDTO.ProfileSkillItems toProfileSkillItems(
            final List<ProfileSkill> profileSkills) {
        List<ProfileSkillItem> items =
                profileSkills.stream().map(this::toProfileSkillItem).toList();

        return ProfileSkillResponseDTO.ProfileSkillItems.builder().profileSkillItems(items).build();
    }

    public List<ProfileSkillItem> profileSkillsToProfileSkillItems(
            final List<ProfileSkill> profileSkills) {
        return profileSkills.stream().map(this::toProfileSkillItem).collect(Collectors.toList());
    }
