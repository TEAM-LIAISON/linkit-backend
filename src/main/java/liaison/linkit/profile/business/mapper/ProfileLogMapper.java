...
public class ProfileLogMapper {
    ...
    public ProfileLogResponseDTO.ProfileLogItems toProfileLogItems(
            final List<ProfileLog> profileLogs) {
        List<ProfileLogItem> items =
                profileLogs.stream().map(this::toProfileLogItem).collect(Collectors.toList());

        return ProfileLogResponseDTO.ProfileLogItems.builder().profileLogItems(items).build();
    }

    public ProfileLogResponseDTO.AddProfileLogResponse toAddProfileLogResponse(
            final ProfileLog profileLog) {
        return ProfileLogResponseDTO.AddProfileLogResponse.builder()
                .profileLogId(profileLog.getId())
                .logTitle(profileLog.getLogTitle())
                .logContent(profileLog.getLogContent())
    ...
    public RemoveProfileLogResponse toRemoveProfileLog(final Long profileLogId) {
        return ProfileLogResponseDTO.RemoveProfileLogResponse.builder()
                .profileLogId(profileLogId)
                .build();
    }

    public ProfileLogResponseDTO.UpdateProfileLogTypeResponse toUpdateProfileLogType(
            final ProfileLog profileLog) {
        return ProfileLogResponseDTO.UpdateProfileLogTypeResponse.builder()
                .profileLogId(profileLog.getId())
                .logType(profileLog.getLogType())
                .build();
    }

    public ProfileLogResponseDTO.UpdateProfileLogPublicStateResponse toUpdateProfileLogPublicState(
            final ProfileLog profileLog) {
        return ProfileLogResponseDTO.UpdateProfileLogPublicStateResponse.builder()
                .profileLogId(profileLog.getId())
                .isLogPublic(profileLog.isLogPublic())
                .build();
    }
    ...
    public AddProfileLogBodyImageResponse toAddProfileLogBodyImageResponse(final Image image) {
        return AddProfileLogBodyImageResponse.builder()
    ...
    public UpdateProfileLogResponse toUpdateProfileLogResponse(final ProfileLog profileLog) {
        return UpdateProfileLogResponse.builder()
                .profileLogId(profileLog.getId())
                .logTitle(profileLog.getLogTitle())
                .logContent(profileLog.getLogContent())
                .isLogPublic(profileLog.isLogPublic())
