...
public class ProfileLogCommentMapper {
        ...
        public ParentCommentResponse toProfileLogCommentResponse(
                ...
                if (comment.isDeleted()) {
                        ...
                        List<ReplyResponse> filteredReplies = replies.stream()
                ...
                if (authorProfile == null || authorProfile.getMember() == null) {
                        // 대댓글 목록 조회 및 변환 (필터링 적용)
                        List<ReplyResponse> replies = profileLogCommentQueryAdapter
                                        .findRepliesByParentCommentId(comment.getId())
                                        .stream()
                                        .map(reply -> toReplyResponse(reply, currentMemberId))
                ...
                String authorName = isQuitAccount
                                ? "(탈퇴한 사용자)"
                ...
                String createdAt = isQuitAccount ? null : DateUtils.formatRelativeTime(comment.getCreatedAt());

                // 현재 사용자가 댓글 작성자인지 확인
                boolean isAuthor = currentMemberId.isPresent()
                                && currentMemberId.get().equals(authorProfile.getMember().getId());

                // 대댓글 목록 조회 및 변환 (필터링 적용)
                List<ReplyResponse> replies = profileLogCommentQueryAdapter
                                .findRepliesByParentCommentId(comment.getId()).stream()
                                .map(reply -> toReplyResponse(reply, currentMemberId))
                                .filter(reply -> reply != null) // null이 아닌 대댓글만 포함
                                .collect(Collectors.toList());
                ...
                return ParentCommentResponse.builder()
                                ...
                                .createdAt(createdAt)
                                .isUpdated(
                                                comment.getModifiedAt().isAfter(comment.getCreatedAt()) ? "true"
                                                                : "false")
        ...
        private ReplyResponse toReplyResponse(
                ...
                String emailId = isQuitAccount ? null : authorProfile.getMember().getEmailId();
                String profileImagePath = isQuitAccount ? null : authorProfile.getProfileImagePath();
                String createdAt = isQuitAccount ? null : DateUtils.formatRelativeTime(reply.getCreatedAt());
         ...
         * @return 변환된 대댓글 응답 DTO
         */
        public ReplyResponse toReplyResponseForOrphanedComment(
                        final ProfileLogComment reply, final Optional<Long> currentMemberId) {
                try {
         ...
         */
        public ProfileLogCommentResponseDTO.UpdateProfileLogCommentResponse toUpdateProfileLogCommentResponse(
                        final ProfileLogComment profileLogComment,
                        final Profile authorProfile,
                        final Long profileLogId) {
        ...
        public ProfileLogCommentResponseDTO.DeleteProfileLogCommentResponse toDeleteProfileLogCommentResponse(
