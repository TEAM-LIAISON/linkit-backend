...
public class ProfileLinkMapper {
    public ProfileLinkResponseDTO.ProfileLinkItem toProfileLinkItem(final ProfileLink profileLink) {
        return ProfileLinkResponseDTO.ProfileLinkItem.builder()
    ...
    public ProfileLinkResponseDTO.ProfileLinkItems toProfileLinkItems(
            final List<ProfileLink> profileLinks) {
        List<ProfileLinkItem> items = profileLinks.stream().map(this::toProfileLinkItem).toList();

        return ProfileLinkResponseDTO.ProfileLinkItems.builder().profileLinkItems(items).build();
    }

    public List<ProfileLinkResponseDTO.ProfileLinkItem> profileLinksToProfileLinkItems(
            final List<ProfileLink> profileLinks) {
        return profileLinks.stream().map(this::toProfileLinkItem).collect(Collectors.toList());
    }

    public ProfileLink toProfileLink(
            Profile profile, ProfileLinkRequestDTO.AddProfileLinkItem requestItem) {
